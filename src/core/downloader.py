...
class DownloadWorker(QRunnable):
    ...
    def __init__(self, task_name: str, url: str, extract_dir: str, suffix: str = ".zip", max_retries: int = 3):
        super().__init__()
    ...
    def run(self):
        """Run the download with retry logic and exponential backoff."""
        last_error = None
        ...
        for attempt in range(self.max_retries + 1):
            try:
                if attempt > 0:
                    ...
                    self.signals.status.emit(f"Retry {attempt}/{self.max_retries} for {self.task_name} in {wait_time}s...")
                ...
                last_error = e
                self.logger.error(f"Download attempt {attempt + 1} failed for {self.task_name}: {e}")
        ...
        # All retries failed
        self.signals.finished.emit(False, f"Failed after {self.max_retries} attempts: {last_error}")
...
class DependencyDownloader(QObject):
    ...
    def _start_worker(self, name: str, url: str, extract_dir: str, suffix: str = ".zip"):
        ...
        worker = DownloadWorker(name, url, extract_dir, suffix, max_retries=3)
